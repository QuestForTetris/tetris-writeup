...
The ROM is currently generated by running a script in Golly that will translate assembly code from your clipboard into a ROM.

## SRL, SL, SRA
These three logic gates are more complicated than your typical AND, OR, XOR, etc.
...
The SRA is slightly different, because we need to copy the sign bit during the shift.
...
This enables us to store one (or more) word of data while waiting for another, allowing for two words of data arriving at different times to be synchronized
...
To do that, we use some SR latches: one latch for each input.
...
## RAM
The RAM was the most complicated part of this project.
It required for very specific control over each SR latch that stored data.
...
Based on the general computer architecture described in the [Overview LINK WILL NEED TO BE UPDATED](THIS IS TEMPORARY), we can construct a working computer!

Downloads:
- [Finished Tetris computer](https://github.com/QuestForTetris/QFT/raw/master/Tetris.zip)
- [ROM creation script, empty computer, and prime finding computer](https://github.com/QuestForTetris/QFT/raw/master/Computer.zip)

![The computer](https://imgur.com/SRnISYL.png)
